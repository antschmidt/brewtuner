...
<style>
  ...
  .login-page-wrapper { /* Reverted to overlay style */
    position: fixed;
    top: 0;
    left: 0;
    right: 0;
...
{:else}
  <div class="landing-hero">
    ...
    <p>BrewTuner helps you dial in the perfect coffee grind each time. Select your roaster, bean, grinder, and brewing method to create and track your grind profiles and results. Log your outcomes and adjustments, then review your grind logs to achieve consistent, delicious coffee.</p>
    <ul class="features-list">
      <li><strong>Personalized Profiles</strong>: Save settings for each coffee and grinder combination.</li>
  ...
  {#if $showAuthOverlay}
    <div class="login-page-wrapper" role="dialog" aria-modal="true" aria-labelledby="auth-dialog-title">
      <div class="login-container">
        <button class="close-auth-overlay" on:click={() => $showAuthOverlay = false} aria-label="Close authentication panel">&times;</button>
        ...
        {#if activeAuthView === 'initial'}
          ...
          <div class="oauth-buttons">
            <button class="oauth-button" on:click={signInWithGoogle} aria-label="Sign in with Google">
              <span>Sign in with Google</span>
            </button>
            <button class="oauth-button" on:click={signInWithGitHub} aria-label="Sign in with GitHub">
        ...
        {:else if activeAuthView === 'emailPassword'}
          ...
          <button class="toggle-auth-mode" on:click={() => activeAuthView = 'initial'}>Back to options</button>
        ...
        {:else if activeAuthView === 'securityKey'}
          ...
          <button class="oauth-button" on:click={isLoginView ? signInWithSecurityKey : signUpWithSecurityKey}>
            {isLoginView ? 'Sign In' : 'Sign Up'} with Security Key
          </button>
          <button class="toggle-auth-mode" on:click={() => activeAuthView = 'initial'}>Back to options</button>
        {/if}
